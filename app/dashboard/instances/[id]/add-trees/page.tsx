...
import { TreePine, Plus, ArrowLeft, Loader2, X } from "lucide-react";
...
export default function AddTreesPage() {
  ...
  const handleAddBulkTrees = async () => {
    ...
    try {
      ...
      await treesApi.bulkCreate(id, units);
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Button asChild variant="ghost" size="icon">
          <Link href={`/dashboard/instances/${id}`}>
            <ArrowLeft className="h-4 w-4" />
          </Link>
        </Button>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Add Trees to Plot</h1>
          <p className="text-muted-foreground">Add one or multiple tree records</p>
        </div>
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-3 justify-end border-t pt-4">
            <Button variant="outline" asChild disabled={submitting}>
              <Link href={`/dashboard/instances/${id}`}>Cancel</Link>
            </Button>
            <Button
